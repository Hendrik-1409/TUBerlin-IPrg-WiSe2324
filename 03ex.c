...
Canvas color_corners(Canvas c) {
    int width = canvas_width(c) - 1;
    int height = canvas_height(c) - 1;
    c = canvas_set_black(c, 0, 0);
    c = canvas_set_black(c, width, 0);
    c = canvas_set_black(c, 0, height);
    c = canvas_set_black(c, width, height);
    return c;
}
