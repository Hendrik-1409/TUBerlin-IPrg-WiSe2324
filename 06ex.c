...
Canvas swatch_red_green(Canvas c) {
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        {
            c = canvas_set_r(c, i, a, normalize(i, width - 1));
            c = canvas_set_g(c, i, a, normalize(a, height - 1));
        }
    }
    return c;
}
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    ...
    for (int i = 0; i < width; i++)
    {
        for (int a = 0; a < height; a++)
        {
            c = canvas_set_channel(c, i, a, channel_x, normalize(i, width - 1));
            c = canvas_set_channel(c, i, a, channel_y, normalize(a, height - 1));
        }
    }
    return c;
}
...
Canvas canvas_to_gray(Canvas c) {
    ...
    for (int i = 0; i < width; i++)
    {
        ...
        {
            RGB color = {
                .r = canvas_get_r(c, i, a),
                .g = canvas_get_g(c, i, a),
                .b = canvas_get_b(c, i, a),
            };
            color = rgb_to_gray(color);
            c = canvas_set_rgb_struct(c, i, a, color);
        }
    }
    return c;
}
