...
int32_t count_fruit_layers(PileOfPancakes p) {
    ...
    {
        FruitLayers = FruitLayers + count_fruit_layers(*(p.further_layers));
    }
    return FruitLayers;
}
...
bool lastLayer(PileOfPancakes p)
{
    ...
    {
        return lastLayer(*(p.further_layers));
    }
    else
    {
        return false;
...
bool is_stable(PileOfPancakes p) {
    if (p.further_layers != NULL && p.layer == ActualPancake && lastLayer(*(p.further_layers)))
    {
        return true;
    }
    else if (p.layer == ActualPancake && p.further_layers == NULL)
    {
        return true;
    }
    return false;
