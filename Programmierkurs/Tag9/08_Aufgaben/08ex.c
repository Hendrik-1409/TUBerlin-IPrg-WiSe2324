...
void turtle_stripes(TurtleCanvas *c) {
    for (size_t i = 0; i < turtle_canvas_height(c) / 2 + 1; i++)
    {
...
uint16_t *find_subarray_starting_at_maximal_number(uint16_t numbers[], size_t numbers_len) {
    u_int16_t *adressMax = find_maximal_number(numbers, numbers_len);
    return adressMax;
}
...
uint16_t *find_minimal_number(uint16_t numbers[], size_t numbers_len) {
    int32_t minNum = numbers[0];
    u_int16_t *number = &numbers[0];
    for (size_t i = 0; i < numbers_len; i++)
    {
        if (numbers[i] < minNum)
        {
...
uint16_t find_maximum_distance(uint16_t numbers[], size_t numbers_len) {
    return *find_maximal_number(numbers, numbers_len) - *find_minimal_number(numbers, numbers_len);
}
...
uint16_t find_minimum_distance(uint16_t numbers[], size_t numbers_len) {
    uint16_t a = 0;
    uint16_t b = 1;
    uint16_t distance = *find_maximal_number(numbers, numbers_len);
    for (size_t i = 0; i < numbers_len; i++)
    {
        for (size_t f = 0; f < numbers_len; f++)
        {
            if (i != f && numbers[f] >= numbers[i] && distance > numbers[f] - numbers[i])
            {
                a = i;
                b = f;
                distance = numbers[b] - numbers[a];
            }
...
void square_ascending(uint16_t numbers[], size_t numbers_len) {
    for (uint16_t i = 1; i <= numbers_len; i++)
    {
        numbers[i - 1] = i * i;
    }
    return;
...
void sort_ascending(uint16_t in[], uint16_t out[], size_t len) {
    ...
    {
        out[i - 1] = *find_maximal_number(in, len);
        uint16_t *maxNum = find_maximal_number(in, len);
        *maxNum = 0;
    }
    return;
}
